...
import { clearErrors, deleteCoupon, getAllCoupons } from "../../redux/actions/couponAction";
...
const CouponList = () => {
    ...
    const { error,coupons} = useSelector((state) => state.allCoupons);
    const { error:deleteError,isDeleted,message} = useSelector((state) => state.deleteCoupon);
            ...
            <table className="table table-borderless table-sm ">
                  ...
                  <tbody className="table-group-divider my-5">
                        {coupons&&coupons.map((coupon,key)=>(
                            ...
                            <td><DateFormatter date={coupon?.expirationTime}/> </td>
                            <td>{coupon?.message}</td>
                            <td>
                            <button class="btn btn-danger" onClick={()=>deleteHandler(coupon?._id)}>Delete</button>
                            </td>
                      </tr>
                        ))}
                  </tbody>
